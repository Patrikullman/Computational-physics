...
int main()
{
    ...
    FILE *fp2 = fopen("energy_data.csv", "w");
    for (int i = 0; i < n_timesteps; ++i){

        for (int j = 0; j <= 3; ++j){
            Ep[i] += kappa/2 * (position_array[i][j+1]-position_array[i][j]) * (position_array[i][j+1]-position_array[i][j]);
        }
    ...
    /* Below is code used for power spectrum */
    double frequencies[n_timesteps];
    for(int i = 0; i < n_timesteps; i++){
	    frequencies[i] = i / (dt * n_timesteps);
    }

    fft_freq(frequencies, dt, n_timesteps);
    fft_freq_shift(frequencies, dt, n_timesteps);

    double fftd_data[n_timesteps];
    powerspectrum(q_1, fftd_data, n_timesteps); 
    powerspectrum_shift(fftd_data, n_timesteps); 
    
    write_to_file("powerspec_three_part_fixed_wall.csv", fftd_data, frequencies, n_timesteps);
