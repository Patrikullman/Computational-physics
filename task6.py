...
A = 0.0311; B = -0.048; C = 0.002; D = -0.0116; gamma = -0.1423; 
beta_1 = 1.0529; beta_2 = 0.3334

epsilon_c = np.zeros((N-1))
for i in range(len(r)):
    if r[i] < 1:
        epsilon_c[i] = A*np.log(r[i]) + B + C*r[i]*np.log(r[i]) + D*r[i]
    else:
        epsilon_c[i] = gamma / (1 + beta_1*np.sqrt(r[i]) + beta_2*r[i])

V_c = epsilon_c + n*np.gradient(epsilon_c)
...
while np.abs(E_diff) > 10**(-5):
    ...
    E, phi = get_phi(r, V_H_vector, V_x, V_c)
