...
atoms_CO = read('Rh_CO.xyz') #read the files that we got when we did task 3, use ase gui traj, insert CO molecule and saved as xyz file. 
atoms_O = read('Rh_O.xyz')

calc = GPAW(xc='PBE', kpts=(4, 4, 1), txt='output_Rh.txt')
atoms_O.set_calculator(calc)
atoms_CO.set_calculator(calc)


relax_Au_CO = BFGS(atoms_CO, trajectory='relax_Rh_CO.traj')
relax_Au_O = BFGS(atoms_O, trajectory='relax_Rh_O.traj')
...
E_surface = -183.1558 #from task 3,  energy of Au  without the adsorbate, this is needed because we need to subtract it from the adsobate energy below.
#surface energy or system energy from task3 ?
...
E_ads_CO = E_CO_energy - (E_surface + 14.210)
...
#now we need to do this for Pt and Rh. 
