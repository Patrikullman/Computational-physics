   ...
   )
{
    ...
    double dt = 0.006; //0.0003
    //energierna och temperatur går bananas för : dt = 0.02,0.03 (dem går bananas efter 0.016 och uppåt))
    //energierna och temperatur ser fina ut för : dt = 
    ...
    */
    ...
    for(int i = 0; i < n_timesteps; ++i){
        ...
        //printf("%f\n", Ek[i]);
        //temp += (2*Ek[i]*1.60218e-19*kB) / (3*N);  //equation 48 in MD pdf 
        //printf("%f\n",temp_inst);
    }
    //temperature = temp / n_timesteps;
    temp = temp_inst / n_timesteps;
    printf("temp:%f\n", temp);
    
double temperature;

temperature = (2*25*1.60218e-19) / (3*4*N*N*N*kB);
printf("T = %f",temperature);


//printf("this is temperature: %f", temperature);


//Lets say we have an array with all our temp_inst. how do we scale the velocities so we can make the T reach 500k  istead of 600-850k ?
